...
import { 
  ...
  ChevronRight, Pill, Clipboard 
} from "lucide-react";

export default function PatientHistoryList() {
  const [patients, setPatients] = useState([]);
  const [fullHistory, setFullHistory] = useState([]); // Stores all prescriptions for this doctor
  ...
  useEffect(() => {
    const fetchDoctorData = async () => {
      try {
        setLoading(true);
        // ✅ Call the new endpoint that filters by the logged-in doctor (Principal)
        const res = await axios.get(`${API_BASE_URL}//doctor/my-patients-history`, axiosConfig());
        const historyData = res.data || [];
        
        setFullHistory(historyData);

        // ✅ Extract unique patients from the prescriptions list for the sidebar
        const uniquePatients = Array.from(
          new Map(historyData.map(item => [item.patient.id, item.patient])).values()
        );
  ...
  const handleViewHistory = (patient) => {
    ...
    // ✅ Filter the already fetched history for the selected patient
    // This avoids an extra API call and ensures only THIS doctor's records are shown
    const patientSpecificHistory = fullHistory.filter(
      (hx) => hx.patient.id === patient.id
    );
    
    setSelectedPatientHistory(patientSpecificHistory);
      ...
      <main className="flex-1 p-8">
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">My Patient Records</h1>
              <p className="text-gray-500 text-sm">Review your clinical medication and dosage history</p>
            </div>
            ...
            <div className="lg:col-span-4 bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden flex flex-col h-[75vh]">
              <div className="divide-y divide-gray-50 overflow-y-auto">
                {filteredPatients.length > 0 ? filteredPatients.map((patient) => (
                  ...
                  <div className="p-10 text-center text-gray-400 text-sm">No patients found.</div>
                )}
              </div>
            </div>
                    ...
                    <div key={hx.id} className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                      <div className="px-6 py-4 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                        <span className="flex items-center gap-2 text-sm font-bold text-gray-600">
                          ...
                          {new Date(hx.createdAt || hx.created_at).toLocaleDateString()}
                        </span>
                        <span className="text-[10px] font-mono text-gray-400 uppercase">Prescription #{hx.id}</span>
                      </div>
                      ...
                      <div className="p-6 space-y-6">
                        <div className="flex gap-4">
                          ...
                          <div className="flex-1">
                            <h4 className="text-[10px] uppercase font-black text-gray-400 tracking-widest mb-1">Medication Details</h4>
                            <p className="text-gray-800 font-bold">{hx.medication || "Standard Treatment"}</p>
                            <div className="mt-2 bg-slate-50 p-3 rounded-xl border border-slate-100">
                              ...
                              <p className="text-sm text-gray-700 font-medium">{hx.dosage || "As prescribed"}</p>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex gap-4">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-gray-600 leading-relaxed italic">
                              "{hx.notes || "No clinical notes provided."}"
                            </p>
                          </div>
                        </div>
                      </div>
